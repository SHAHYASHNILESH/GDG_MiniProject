...
def drawPKButtons(app, canvas):
    ...
    canvas.create_text(
        375,
        700,
        text="""Generate a new maze with either  
  Prim's or Kruskals's algorithm.""",
        fill="Black",
    )
...
def drawOHQStudent(app, canvas):
    ...
    canvas.create_image(
        app.ohqStudentX, app.ohqStudentY, image=getCachedPhotoImage(app, ohqStudent)
    )
...
def drawMazeWalls(app, canvas):
    ...
    for (x0, y0), (x1, y1) in app.listOfPaths:
        canvas.create_rectangle(x0, y0, x1, y1, fill="cadetblue3", outline="cadetblue3")
    ...
    if app.drawSolutionBool == True:
        if app.listOfSolutionPaths != None:
            for (x0, y0), (x1, y1) in app.listOfSolutionPaths:
                canvas.create_rectangle(
                    x0, y0, x1, y1, fill="light yellow", outline="light yellow"
                )
...
def drawPopUpGameOver(app, canvas):
    ...
    if app.showPopUp == True:
        ...
        canvas.create_text(
            app.width / 2,
            app.height / 2,
            text=f"""You have reached the OHQ Student. Score = {app.health}
                  Press r to reset the game.
              Press q to play the bonus round.""",
            fill="Black",
        )
...
def drawStartStopBoxes(app, canvas):
    ...
    canvas.create_rectangle(
        lowerX, lowerY, upperX, upperY, fill="white", outline="white"
    )
    # stop:
...
def drawGhostStudentSolution3(app, canvas):
    if app.listOfBacktrackerSolutionPaths3 != None:
        for (x0, y0), (x1, y1) in app.listOfBacktrackerSolutionPaths3:
            canvas.create_rectangle(
                x0, y0, x1, y1, fill="light green", outline="light green"
            )
    r = 10
...
def drawHealth(app, canvas):
    ...
    if app.drawHealthMessage == True:
        canvas.create_text(
            x, y + 30, text="-10 Health!", fill="Red", font="Helvetica 20 bold"
        )
