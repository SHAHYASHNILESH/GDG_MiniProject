...
def homeScreenMode_drawWelcome(app, canvas, rustyPurple, mediumPurple, darkPurple):
    ...
    canvas.create_text(
        app.width / 2,
        75,
        text="""     Welcome to Maze Simulator!""",
...
name in this text editor""",
        font="Courier 14",
    )
    # draw text:
    for char, x in app.value:
        canvas.create_text(x, app.textY + 10, text=char, font="Courier 20")
    # draw cursor:
...
def homeScreenMode_drawLeaderboard(app, canvas, beige, darkerBeige, brown):
    ...
    canvas.create_rectangle(
        250 + 200, 400, 550 + 200, 460, fill=darkerBeige, outline=brown
    )
...
def helpMode_redrawAll(app, canvas):
    ...
    font = "Times 12 bold"
    canvas.create_text(
        app.width / 2, 2 * app.height / 10, text="Rules", font="Times 16 bold"
    )
    canvas.create_text(
        app.width / 2 + 10,
        app.height / 2 - 150,
...
""",
    ...
    canvas.create_text(
        app.width / 2, app.height / 2, text="Shortcut commands", font="Times 14 bold"
    )
    canvas.create_text(
        app.width / 2,
        app.height / 2 + 150,
...
""",
    ...
    canvas.create_text(
        app.width / 2,
        9 * app.height / 10,
        text="Press any key to return back to the game!",
        font=font,
    )
...
def pickYourTAMode_mousePressed(app, event):
    ...
    if (
        app.width / 3 - 20 <= event.x <= app.width / 3 + 20
        and app.height / 2 - 25 <= event.y <= app.height / 2 + 30
    ):
        app.ta = "winston"
    # if clicked on stephanie
    if (
        2 * app.width / 3 - 20 <= event.x <= 2 * app.width / 3 + 20
        and app.height / 2 - 25 <= event.y <= app.height / 2 + 30
    ):
        app.ta = "stephanie"
...
def pickYourTAMode_redrawAll(app, canvas):
    ...
    canvas.create_text(
        app.width / 2, app.height / 6, text="Pick Your TA!", font="Times 20"
    )
    canvas.create_text(
        app.width / 2,
        app.height / 6 + 20,
        text="Click the TA to select.",
        font="Times 20",
    )

    canvas.create_text(
        app.width / 3, app.height / 2 - 50, text="Winston", font="Times 20"
    )
    canvas.create_text(
        2 * app.width / 3, app.height / 2 - 50, text="Stephanie", font="Times 20"
    )
    ...
    # see citation 3
    canvas.create_image(
        app.width / 3, app.height / 2, image=getCachedPhotoImage(app, app.winstonIcon)
    )
    canvas.create_image(
        2 * app.width / 3,
        app.height / 2,
...
def removeStudent(app):
    ...
    for x0, y0, x1, y1 in app.listOfTiles:
        ...
        if (
            tileCenterX - 10 <= app.TA.playerX <= tileCenterX + 10
            and tileCenterY - 10 <= app.TA.playerY <= tileCenterY + 10
        ):
...
def keyPressedHelperBonus(app, event):
    if event.key == "Left":
        ...
        if (
            app.margin < app.TA.playerX < app.width - app.margin
            and app.margin < app.TA.playerY < app.height - app.margin
        ):
            for x0, y0, x1, y1 in app.listOfTiles:
                if min(x0, x1) + 5 <= app.TA.playerX <= max(x0, x1) + 5 and min(
                    y0, y1
                ) <= app.TA.playerY <= max(y0, y1):
                    isOnAPath = True
                    break
            if isOnAPath == False:
                app.TA.movePlayer(10, 0)
        ...
        if (
            ...
            for x0, y0, x1, y1 in app.listOfTiles:
                if (
                    min(x0, x1) <= app.TA.playerX <= max(x0, x1)
                    and min(y0, y1) + 5 <= app.TA.playerY <= max(y0, y1) + 5
                ):
                    isOnAPath = True
                    break
            if isOnAPath == False:
...
you get an extra +10 health!""",
        font="Times 20",
    )

    canvas.create_text(
        app.width / 4, 14 * app.height / 15 - 20, text="Press r to reset game."
    )
    canvas.create_rectangle(
        app.width / 2 - 50,
        14 * app.height / 15 - 40,
...
def drawTimeUpPopUp(app, canvas):
    ...
    canvas.create_text(
        app.width / 2, app.height / 2 - 20, text="Time up!", font="Times 20"
    )
    canvas.create_text(
        app.width / 2, app.height / 2, text=f"Health = {app.health}", font="Times 20"
    )
    canvas.create_text(
        app.width / 2,
        app.height / 2 + 20,
        text=f"Press h to go back to the Home Screen.",
        font="Times 15",
    )
...
def drawTA(app, canvas):
    ...
    canvas.create_image(
        app.TA.playerX, app.TA.playerY, image=getCachedPhotoImage(app, spriteImage)
    )
...
def alterListOfPaths(app, listPaths):
    ...
    for i in range(len(listPaths) - 1):
        ...
        listPaths[i] = [(newX0 - 15, newY0 - 15), (newX1 + 15, newY1 + 15)]
...
def makeBoard(app):
    ...
    app.listOfSolutionPaths = solution(
        app, app.listOfPaths, (0, 0), (app.rows - 1, app.cols - 2)
    )
    ...
    if app.listOfSolutionPaths != None:
        ...
        for i in range(len(app.listOfSolutionPaths)):
            ...
            newX1 = app.margin + x1 * cellWidth
            newY1 = app.margin + y1 * cellHeight
            app.listOfSolutionCoords.append([(newX0, newY0), (newX1, newY1)])
            app.listOfSolutionPaths[i] = [
                (newX0 - 10, newY0 - 10),
                (newX1 + 10, newY1 + 10),
            ]
    else:
...
def makeIconImages(app):
    # winston:
    winston = "Codebase/images/spriteSheet-trans1.png"
    winston = app.loadImage(winston)
    winston = app.scaleImage(winston, 0.5)
    # stephanie
    stephanie = "Codebase/images/spriteSheet-trans2.png"
...
def makeBonusStudents(app):
    app.bonusStudent = "Codebase/images/Student.png"
...
def makeOHQStudent(app):
    ...
    app.ohqStudentImage = "Codebase/images/idle-student-transparent.png"
